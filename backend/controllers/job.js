...
const createJob = async (req, res) => {
    try {
        ...
        const errors = [];
        if (!formData.title || !String(formData.title).trim()) errors.push("title is required");
        if (!formData.category) errors.push("category is required");
        if (!formData.description || String(formData.description).trim().length < 50)
            errors.push("description is required and must be at least 50 characters");
        if (!formData.budget && formData.budget !== 0) errors.push("budget is required");
        if (!formData.location || !String(formData.location).trim()) errors.push("location is required");
        if (!formData.pincode || !String(formData.pincode).trim()) errors.push("pincode is required");
        ...
        const latitudeProvided = formData.latitude !== undefined && formData.latitude !== "";
        const longitudeProvided = formData.longitude !== undefined && formData.longitude !== "";
        ...
        const jobPayload = {
            ...
            shift_preference: formData.timePreference || formData.shift_preference || "flexible",
